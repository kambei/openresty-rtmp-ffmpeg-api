...
function session.new(opts)
    ...
    local ide, iden = prequire("resty.openidc.session.identifiers.", opts.identifier or defaults.identifier, "random")
    local ser, sern = prequire("resty.openidc.session.serializers.", opts.serializer or defaults.serializer, "json")
    local com, comn = prequire("resty.openidc.session.compressors.", opts.compressor or defaults.compressor, "none")
    local enc, encn = prequire("resty.openidc.session.encoders.",    opts.encoder    or defaults.encoder,    "base64")
    local cip, cipn = prequire("resty.openidc.session.ciphers.",     opts.cipher     or defaults.cipher,     "aes")
    local sto, ston = prequire("resty.openidc.session.storage.",     opts.storage    or defaults.storage,    "cookie")
    local str, strn = prequire("resty.openidc.session.strategies.",  opts.strategy   or defaults.strategy,   "default")
    local hma, hman = prequire("resty.openidc.session.hmac.",        opts.hmac       or defaults.hmac,       "sha1")
